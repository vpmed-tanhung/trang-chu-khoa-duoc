(function () {
  ...
  function openView(name) {
    if (window.VPMED_PLATFORM && typeof window.VPMED_PLATFORM.openFeature === 'function') {
      window.VPMED_PLATFORM.openFeature(name, {source:'consultation-module'});
      return;
    }
  ...
  function installView() {
    ...
    const main = document.getElementById('clinical-app-shell');
  ...
  function init() {
    ...
    installView();
    render();
    if (location.hash === '#clinical-' + MODULE_ID) openView(MODULE_ID);
