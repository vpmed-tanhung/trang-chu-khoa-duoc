(() => {
  ...
  class VpmedPharmacovigilance extends HTMLElement {
    ...
    openDetail(id) {
      ...
      this.$('#modalBody').innerHTML = `<div class="detail-grid">
        ...
        <div class="box full source-box"><h4>Nguồn thông tin</h4><p><b>${escapeHtml(item.source || 'Nguồn chính thức')}</b></p>${item.source_note ? `<p><b>Nguồn được bài gốc trích dẫn:</b> ${escapeHtml(item.source_note)}</p>` : ''}<p class="source-link"><a href="${escapeHtml(item.source_url || item.url)}" target="_blank" rel="noopener noreferrer">Mở trực tiếp bài nguồn ↗</a></p></div>
      </div>`;
  ...
  function showView(name) {
    if (window.VPMED_PLATFORM && typeof window.VPMED_PLATFORM.openFeature === 'function') {
      window.VPMED_PLATFORM.openFeature(name, {source:'pharmacovigilance-module'});
      return;
    }
    const target = document.getElementById(`view-${name}`);
    if (!target) return;
    document.querySelectorAll('#clinical-workspace .view').forEach((view) => view.classList.remove('active'));
    target.classList.add('active');
    document.querySelectorAll('#main-nav [data-view]').forEach((button) => button.classList.toggle('active', button.dataset.view === name));
    history.replaceState(null, '', `#clinical-${name}`);
    document.getElementById('main-nav')?.classList.remove('open');
    document.getElementById('cong-cu-lam-sang')?.scrollIntoView({behavior:'smooth',block:'start'});
  }
  ...
  function install() {
    ...
    if (!section) {
      ...
      const appShell = document.getElementById('clinical-app-shell');
    ...
    if (location.hash === `#clinical-${VIEW_NAME}`) showView(VIEW_NAME);
    window.addEventListener('hashchange', () => { if (location.hash === `#clinical-${VIEW_NAME}`) showView(VIEW_NAME); });
  }
