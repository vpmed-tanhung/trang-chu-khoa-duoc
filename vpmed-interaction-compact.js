...
(function () {
  ...
  function getInteractions() {
    return window.VPMED_QD5948?.rules || window.VPMED_INTERACTIONS || [];
  }
  ...
  function isPairMatch(item, a, b) {
    if (window.VPMED_QD5948?.matchPair) return window.VPMED_QD5948.matchPair(a, b, item);
    const d1 = norm(item.drug1);
    const d2 = norm(item.drug2);
    const na = norm(a);
    const nb = norm(b);
    return (na === d1 && nb === d2) || (na === d2 && nb === d1);
  }
    ...
    const rows = matches.map((m, idx) => {
      ...
      return `
        <div class="vpmed-md-hit">
          ...
          <div><b>Cặp hoạt chất:</b> ${escapeHtml(i.drug1 || m.a)} + ${escapeHtml(i.drug2 || m.b)}</div>
          ${consequence ? `<div><b>Hậu quả tương tác:</b> ${escapeHtml(consequence)}</div>` : ''}
          ${mechanism ? `<div><b>Cơ chế:</b> ${escapeHtml(mechanism)}</div>` : ''}
          ${management ? `<div><b>Khuyến cáo xử trí:</b> ${escapeHtml(management)}</div>` : ''}
          <div><b>Nguồn:</b> Quyết định 5948/QĐ-BYT</div>
        </div>`;
    }).join('');
